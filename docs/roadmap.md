...
- [ ] **Hacking & The Grid — Plan 7+ (multi-track)** — see `docs/plans/2026-05-03-plan-7-roadmap.md` for the full sub-project map. Summary:
  - **Plan 7 — Device Shells.** Per-`docs/superpowers/specs/2026-05-01-device-shells-design.md`. Diegetic CLI shells for every electronic fixture; tag-driven command set; guest/root auth; long-channels; pivot vs wired doorways. Subsumes the old plan's nmap-from-Grid binding, help overlay, lore-archive viewer, hostile-QH model, PrecursorConsole AlienTech.
    - **Phase A (in progress, on `feature/device-shells`):** Hackable schema bumped (firmware_state, cracked_digits, escalated, dumped_bytes; save v62→v63). HackCommand + HackCommandRegistry + HackChannel + DeviceShell skeletons. Civilian flavor pack (universal fallback) + FixtureOsId for Door/Camera/Vending/Console/Locker/Power. Universals (`help`, `whoami`, `clear`, `history`, `exit`, plus `<cmd> --help`). `pda> ssh [<user>@]<ip>` with strict manual semantics (root@locked rejects). `(hack) Shell Access` interactable on Electronic Hackables (autoruns smart-ssh). Real-world wired-in body state: cyan-pulse `@`, movement/attack/use blocked, Esc yanks cable. Phase B will add Tron-window swap, hashcat/dump partial-state, full Locked/HasOptics/Weaponized/PowerNode/Mobile/DataStore command sets, Corp + Cartel flavor packs, Aerojack/Untether items, ColdHands/RootKit skills, AlienTech reject path, and Plan 5 `nmap -m b` removal.
