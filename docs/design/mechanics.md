...
**Tactical combat — COMBAT/NORMAL mode (Slice 5-1).** Outside combat you move freely (NORMAL). When a node directly pipe-connected to yours holds a live hostile (non-White) ICE you auto-enter COMBAT: a `⚠ COMBAT` caption shows, free movement is disabled (the arm/`Tab`/`Space` cast flow stays available), and you return to NORMAL when the connected nodes are clear, on hard jack-out, or via dev `:net combat off`. White ICE is ambient and never locks; charmed (DaemonHijack'd) ICE do not lock. Dev: `:net combat on|off` forces the mode.

**CORE actions — round model (Slice 5-2).** Each cyberdeck carries up to four intrinsic **CORE actions** (`q`/`w`/`e`/`r`, index 0–3). The action set is deck-defined and immutable per deck; `None` (empty) slots are dim in the panel and inert on keypress. In COMBAT mode, pressing `q`/`w`/`e`/`r` fires the corresponding CORE action as **one world beat** (returns `true`, same as a successful CAST). The CAST arm/`Tab`/`Space` flow and `.` wait are unaffected. The DECK panel shows a right-hand CORE column — keycap, glyph, and label drawn in the action's color. Current deck assignments: **Pidgin Mark I** → `{SNIFF, CHANNEL, —, —}`; **Polyglot DCK-2** → `{SNIFF, CHANNEL, BRACE, RUN}`. Slice 5-2 stub behaviors: **SNIFF** logs live ICE count + in-flight count; **CHANNEL** adds +2 RAM (clamped to `ram_max`); **BRACE** sets `brace_turns = 1` (1-beat mitigation flag, consumed by future defensive logic); **RUN** logs "autopilot" (auto-resolve = Slice 6). Real SNIFF target reveal and RUN auto-resolve land in Slice 6.

**Tactical combat — ranged ICE casters (Slice 5-3).** Gray ICE no longer melee/chase in netspace. While COMBAT-locked, a Gray sitting in a node directly pipe-connected to yours casts a single-target payload down that pipe at your node every `kIceCastCadence` beats; it travels the pipe (one segment per beat, like your payloads) and resolves as avatar damage at your node — the shared payload engine, run in reverse (origin = ICE node, target = your node). White ICE stays ambient (never casts); Black is unchanged this slice (the pipe-graph walker is Slice 5-4). **BRACE** (CORE) now does something: it halves the next hostile impact (rounded down, min 1) and is consumed. The cast kit is the balance knob — `kIceGrayCastDamage` / `kIceGrayCastRadius` / `kIceCastCadence` in `net_combat.h` (default `1 / 0 / 3`). Note: at the default 1 damage, BRACE's halve-min-1 is a deliberate no-op; raise `kIceGrayCastDamage` to ≥ 2 (a higher-tier tuning lever) for BRACE to mitigate. Hostile payloads carry a `NetInFlight::hostile` ownership flag (also the hook S4 collision keys off). A Gray killing the avatar routes the existing `NonBlackDeath` "AVATAR LOST" path, not the Black net+meat game-over.

**Tactical combat — pipe collision + node-scoped Impact (Slice 5-4).** Opposing payloads on the same pipe now collide: when your payload and an ICE payload meet or cross a cell on the same beat (positions within one cell, approaching), they interact — equal damage annihilates both; otherwise the loser is destroyed and the winner continues carrying the damage difference (its later collisions and node Impact use the reduced value). Weight is the payload's damage; AOE radius is a node-Impact property, ignored in-pipe. Payload↔Black-walker is a named seam wired in Slice 5-5. Separately, a payload's node Impact is now **node-scoped**: it resolves against the room containing the pipe terminus — single-target hits the closest live ICE anywhere in that room (fixing the old radius-0 "must stand exactly on the terminus" miss), AOE hits all live ICE in the room, RELAY chains from the struck ICE. Known S4 simplification: a multi-payload LOOP shares one damage value, so a collision that reduces it weakens that entry's remaining payloads too — per-payload damage is a future refinement if it matters.

**Tactical combat — Black walker + unconditional game-over (Slice 5-5).** Black ICE no longer tile-chases or melees. It is now a **pipe-graph walker**: at a node it picks the next-hop pipe along a shortest path toward your room (BFS over the pipe graph) and starts stepping one pipe-cell per beat; in-pipe it advances one cell per beat. While in transit it is a **collidable** — your payload meeting it (same met-or-crossed rule as payload↔payload) is consumed and Black takes that payload's damage (same-as-node-Impact). Black is therefore killable in three ways: a node Impact at its current room (existing single-target / AOE), a pipe collision while it transits, or kill it before it leaves (room Impact). If a Black walker steps into your room, it's **unconditional game-over** in net AND meat — the "AVATAR LOST" freeze plays and `GameState::GameOver` fires regardless of your meat HP. (Pre-S5 BlackIceDeath had a survivable bleed-through branch; that's removed — post-S5 Black has no tile-melee and the walker reaching you is *the* failure state.) The boss-clock pressure is the steady 1-cell/beat advance: every beat you don't kill or escape it, it's one cell closer.

**Tactical combat — Telegraph + SNIFF tier ladder + RUN autopilot (Slice 5-6).** Enemy intent is now **legible**. Gray ICE no longer spawns a payload the instant its cooldown expires — it enters a **windup phase** (4 beats by default), telegraphed to the player as a progress bar `gray.exe [████░░░░] 0/4` next to the ICE glyph and in the new HOSTILES band. When the bar fills, the payload launches; the player has 4 + seg_len (6–10) beats from "charging" to node-Impact. The **SNIFF** core action is now a sticky tier ladder: each press raises `sniff_level` (capped at 2) AND consumes the beat. Tier 1 reveals payload damage, ICE cast progress, and Black's coarse proximity (near/far). Tier 2 adds ICE program names, ICE current HP, and Black's precise beats-to-your-room ETA. Reveals stay until you jack out — they're an investment, not a one-shot. The per-ICE **obfuscation tier** (Watchdog / Elite / Boss / Blackwall) is shipped as an enum seam; all S6 ICE are Watchdog (full transparency available at max SNIFF), with the other tiers reserved for future content. **RUN** is now a real Game-touching auto-resolve loop: one R press ticks `tick_grid` up to 64 beats, halting the first beat a live Black walker becomes one pipe-hop from your room (the edge-trigger boss-clock alarm). It does NOT interrupt on hostile payloads, damage taken, or pipe collisions in v1 — a deliberate "calculated risk" framing. Telegraph rendering ships in TWO places (inline beside the ICE/§, AND a dedicated HOSTILES band between DECK and VITALS); the design author A/B-decides which to keep at slice boundary.

**Tactical combat — Door daemons (Slice 7c.1).** The door netspace's LOCK and BOLT objectives are now first-class **daemons** instead of static breakwall density / lone-room glyph. LOCK becomes a `LOCK.fw` daemon per LOCK room (DarkGray, RoomFill render — its HP visualizes as the existing breakwall density gradient `▓ → ▒ → ░ → ·`; HP scales with tier 4–6). BOLT becomes a `BOLT.T9` daemon (Yellow `▣` glyph, Glyph render — a micro-boss with HP 12-36 across tiers, longer windup, doubled cast damage). Both fire programs back at the player per the S6 caster pipeline. Killing a LOCK clears its room's wall; killing the BOLT opens the door in meatspace. Mechanically: a new `DaemonKind` enum + `DaemonDef` table (`include/astra/daemon.h`, `src/daemon.cpp`) sits on top of the existing IceColor archetype; the kind drives name, glyph, color, base stats, and render style; the archetype still drives the behavior path (Gray = caster, White = patrol, Black = walker — no new code paths). Default `Watchdog` kind matches the S6 hardcoded Gray, preserving existing combat-bench behavior. Future grammar slices add ATM/Camera/Corpse/Elevator/Turret/Vending daemons through the same table.

**Tactical combat — Grammar daemon sweep (Slice 7c.2).** The remaining six netspace grammars get typed daemon coverage following the S7c.1 door pattern. New DaemonKind entries: VAULT.fw + TELLR.K9 + FRAUD.exe + PKT.dat (ATM), LENS.cam + ARCHIVE.K9 (CAMERA), MEMRY.kex (CORPSE), FLOOR.K9 + SCRTY.fw + HOUSE.K9 (ELEVATOR), TRRT.dat (TURRET), LOL.bin (VENDING — ultra-rare 1-in-64 easter egg). A third render style, `BreakwallTile`, paints a single-cell density gradient at (ice.x, ice.y) for ELEVATOR's SECURITY gate. `NetSpawnSpec` gains a `DaemonKind kind` field so grammar triggers (ATM's FRAUD enforcer + trace≥100 PACKETS wave) spawn typed daemons. `seed_daemon` promotes from a per-grammar local helper to `include/astra/grammars/seed_daemon.h` (+ `seed_daemon_at` for single-tile placement + `seed_daemon_in_room_at` for arbitrary interior coords). No new behavior code paths — every new daemon rides the existing Gray-caster pipeline. Per-grammar tier-scaling tables follow the kLockTiers/kBoltTiers shape.

**Tactical combat — Spawn-site discipline (Slice 7d).** S7c.2's grammar daemon sweep surfaced five ICE-spawn violations where daemons landed either in the player's jack-in room (ATM FRAUD enforcer, VENDING LOL.bin) or in non-room cells (ATM PACKETS in the `$` ring decoration, ELEVATOR SCRTY.fw in the spine pipe gap, TURRET TRRT.dat in the JACK→AMMO corridor) — combat's pipe-only engagement model leaves all five inert. S7d's rule: every ICE constructed at gen time or spawned by a trigger must lie inside some `NetRoom` AND not in the player's jack-in room. Fixes: ATM FRAUD → BALANCE; ATM PKT.dat → explicit pool of BALANCE+VAULT interior cells (AUTH excluded); VENDING LOL.bin → a shelf room picked by `desc.seed`; ELEVATOR SCRTY.fw → floor mid+1 (the floor ABOVE the breakwall) with a new `Ice.gate_tile_x/y` field tagging the gap cell, and a `tick_grid` post-impacts hook that flips the gap to Floor on SCRTY.fw's death (the daemon IS the gate); TURRET TRRT.dat → AMMO interior. `DaemonRenderStyle::BreakwallTile` retires (no remaining users). The combat model itself is unchanged; the spec was correct, the violations were at spawn sites.

**Tactical combat — Pipe registration, camera, and uncapped travel (Slice 7e).** Three independent fixes surfaced during S7d in-game testing. (1) `NetCamera::follow` oscillated vertically when the playfield exceeded the viewport — caused by `deadzone_margin = 12` exceeding `viewport_h / 2` so the "stable region" between scroll branches was empty. Fix: cap effective deadzone to `(viewport - 1) / 2`. (2) ATM and CORPSE grammars hand-stamped pipe TILES via `hrun`/`vrun` lambdas but never pushed `NetPipe` entries to `b.ns.pipes`, leaving `combat_should_lock` and player-cast-aim structurally inert in both grammars since they shipped. Fix: register 2 NetPipes for ATM (AUTH↔BALANCE, BALANCE↔VAULT) and 4 for CORPSE (JACK↔MEMORY, MEMORY↔PROG, JACK↔STASH spine, PROG↔GHOST spine). LAST RUN tees deferred — that room has no daemons. Defense-in-depth: trigger eval in `tick_grid` §1b now skips cells in the avatar's current room (same-room ICE never engages); ATM's FRAUD candidate list widens to span BALANCE + VAULT so the dynamic guard has a viable fallback. (3) `clamp_seg_len`'s `[2, 6]` upper bound dropped — payload travel now scales 1:1 with physical pipe length, making pipe geometry a real tactical lever.

**Tactical combat — CORPSE spine split (Slice 7f).** S7e's CORPSE pipe registration left LAST RUN (the hub) unable to cast — the col0 and col2 spines were registered as single straight JACK↔STASH and PROG↔GHOST `NetPipe` entries, so the hub had zero connected pipes. User-rule (2026-05-28): one physical pipe with a junction = N cast paths from the perspective of the junction room. Fix: split each spine into TWO `NetPipe` entries that route through the hub junction — JACK↔LAST_RUN + LAST_RUN↔STASH (col0); PROG↔LAST_RUN + LAST_RUN↔GHOST (col2). Six total registered pipes in CORPSE now (top-row ×2 + split spines ×4). From the hub the avatar Tab-cycles through 4 cast paths; from JACK or PROG, 2; from STASH or GHOST, 1. Tile-paint unchanged — only the engine's pipe data model changes.

**Tactical combat — `room_index_at` reverse-iteration (Slice 7g).** VENDING's LOL.bin hostile payload visually travelled AVATAR→ICE instead of ICE→avatar. Root cause: VENDING's `add_room_outline` registers an OUTER decorative frame at `rooms[0]` whose rect strictly contains every interior cell. `room_index_at` iterated rooms 0→N-1 and returned the FIRST containing rect — so LOL.bin, the avatar, and pipe endpoints ALL resolved to OUTER instead of their actual rooms. In `pipe_path_cells`, `rA == rB == rav` then hit the degenerate "all-same-room" branch which returned cells WITHOUT reversing, causing `ice_cast_tick`'s subsequent `reverse(path)` to flip the orientation backwards. Fix: iterate `ns.rooms` in REVERSE registration order in both `room_index_at` and `room_index_at_strict`'s first (containment) pass. Later-registered (typically nested / smaller) rooms now win containment over outer wrapper frames. All shipped grammars except VENDING have disjoint rooms — reverse iteration is a no-op for them. Side benefits: telegraph block placement now correctly anchors to inner rooms; `home_room_idx` for any future patrolling ICE in VENDING resolves correctly.
