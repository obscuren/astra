...
- [x] **Tinkering expansion** — 24-material catalog across 3 tiers, junk dual-use + 9 refinement recipes, hand-tuned synthesis costs, schematic-based crafting for 16 inert consumables (stims/grenades/mines), tier-weighted salvage, `tinkering.md` reference (2026-04-27)
...
- [x] **Stage 4 infra — EventBus + scenario graph** — in-process typed event bus, effect primitives, scenario registration (2026-04-18, architectural reference: [`technical/scenario-graph.md`](../technical/scenario-graph.md))
...
- [x] **Space station types** — Normal/Scav/Pirate/Abandoned/Infested with unique per-station keepers and specialty rooms
...
- [x] **PDA refactor** — `character_screen` renamed to `pda_screen`; per-tab modules (one `pda_<tab>_tab.cpp` per tab); new (placeholder) Hacking tab. Foundation for the Relay Network gameplay layer.
...
- [~] **The Net / Relay Network — netspace redesign** — major reframe of jack-in geography. Each hackable target opens its own procgen netspace (door, vending, camera, ATM, turret, NPC head, mainframe, Blackwall tear, …) with its own shape grammar, ICE profile, and visual language. Replaces the prior single-Site / Imprint design end-to-end. Driving docs:
  ...
  - [x] **Phase 0 — Demolition + scaffold** *(2026-05-12)*. Imprint mechanic, dead-implant stub, legacy sector generators (`lan_sector_generator`, `grid_room_templates`, `deep_grid_sector`, `grid_anchor_layout`), persistence layer (`grid_persistence`, `grid_zone_overlay`, `sector_runtime_state`), and `GridNetwork` graph all retired. `Netspace` abstraction + empty-room stub generator ship as the new substrate. `Grid*` → `Net*` rename across the netspace stack. Save schema v76 → v77.
  - [x] **Phase 1 — Visual language + door/vending/camera grammars** *(2026-05-12)*. `NetTile` vocab extended (wall density gradient `· ░ ▒ ▓ █`, box-drawing borders, animated pipes, glyph escape). `NetRoom` + `NetPipe` primitives; `NetspaceBuilder` DSL. Three grammars to design-doc fidelity: `gen_door_netspace` (linear lock chain with density gradient), `gen_vending_netspace` (3 shelf nodes draining into DISPENSE), `gen_camera_netspace` (5 lens bank → FEED → ARCHIVE + DVR with scan-line ambient anim). `TargetDescriptor` dispatch wires jack_in to grammars; door/vending fixtures in the meatworld test bed open the matching netspace. Title bar polish (subtitle, tier badge, time-dilation indicator).
  - [x] **Phase 2 — Wall degradation + animation** *(2026-05-16)*. `NetTile::Breakwall` fixture + `BreakwallGroup` state machine; `Breach.exe` verb breaks walls. 800→300ms hit glitch animation on `AnimationManager`. Vending grammar visual pass: pipe-port tiles (`╨ ╜ ╦`), `add_room_outline`, `connect_vertical`.
  - [x] **Phase 3 — Window state machine + jack-in ritual** *(2026-05-16)*. Trace bands Stable < 40 / Stressed 40–70 / Hunted 70–95 / Critical 95+ with hysteresis-5; black ICE pins Critical. State-driven render modes on the In-Net UI window: Stressed = title-bar flicker; Hunted = border §-crawl + HP digits shimmer-lie + command-line glitch; Critical = + RAM deception + layout-fragment flicker-out; ~1s Black-ICE "YOU ARE SEEN" full-window takeover. Vitals lie is display-only; sim stays honest. Jack-in Opening ritual (6-frame, wall-clock, skippable after first run). Jack-out variants: normal reverse close; Q/HardJackOut panic (red flash + `~@~` meatworld frame); avatar death forced "AVATAR LOST" freeze. `WindowSequence` on `NetSession`; sound stubs.
- [~] **Legacy Site / LAN layout** — *retired.* Sector generators, room templates, GridNetwork graph, and the Imprint mechanic were demolished in Phase 0 of the netspace redesign; per-target grammars replace them.
