...
// Deep-grid Atlas anchor registration retired with the netspace redesign;
// the deep-grid hub sector is gone. Kept as a stub so program_effects'
// "breach DeepGridGateway" path still links; it now no-ops. Phase 0 stub
// netspace has no DeepGridGateway tiles, so this is unreachable in practice.
bool register_deep_grid_warp_anchor(WorldManager& /*world*/,
                                    GridNodeId /*lan_root_id*/) {
    return false;
}
