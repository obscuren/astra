...
// ---------------------------------------------------------------------------
// old_impl — legacy RuinGenerator-backed body, used only for Conclave Archive
// until that dungeon is migrated to the new pipeline.
// ---------------------------------------------------------------------------
namespace old_impl {
...
std::pair<int, int> find_stairs_up(const TileMap& m) {
    // Delegate into old_impl's anonymous-namespace helper via the legacy
    // fixture-scan. This function is defined in old_impl's anon namespace
    // but we can reimplement it inline here identically — it has no
    // dependency on old_impl state.
    const auto& ids = m.fixture_ids();
    const int w = m.width();
    const int h = m.height();
    for (int y = 0; y < h; ++y) {
...
void generate_dungeon_level(TileMap& map,
    ...
    // TODO[archive-migration]: once the Conclave Archive is ported to the
    // new pipeline, remove this branch and delete old_impl entirely.
